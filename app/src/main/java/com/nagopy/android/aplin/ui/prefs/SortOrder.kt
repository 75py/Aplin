...
enum class SortOrder(
    ...
    AppName(R.string.pref_sort_order_app_name) {
        override fun sort(packages: List<PackageModel>): List<PackageModel> =
            packages.sortedWith(compareBy({ it.label }, { it.packageName }))
    },
    AppPackageName(R.string.pref_sort_order_app_package_name) {
        override fun sort(packages: List<PackageModel>): List<PackageModel> = packages.sortedWith(compareBy { it.packageName })
    },
